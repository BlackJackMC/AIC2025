...
CLIP_FEATURES_PATH = "dataset/clipv2"
...
def search_all_queries(queries, k):
    with torch.no_grad():
        # Use open-clip's tokenizer to tokenize queries
        text_features = [model.encode_text(tokenizer(query).to(device)).view(1, -1) for query in queries]
